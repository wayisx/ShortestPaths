...
//import de.unikiel.npr.thorup.algs.Thorup.ComponentTree.TreeNode;
...
public class Thorup2 {
	...
	public void cleanUpBetweenQueries() {
			// reset S
			s = new boolean[n];
	...
	private void expand(ComponentTree.TreeNode v) {
		...
		v.ix0 = t.getMinDviMinus(v) >> (v.i - 1);
		v.ix8 = v.ix0 + v.delta;
		//System.out.println("ix0:"+v.ix8+" delta:"+v.delta );
		// bucket the children of [v_i]
		v.initializeBuckets();
		t.deleteRoot(v);
		
		for (ComponentTree.TreeNode wh : v.children) {
			int min = t.getMinDviMinus(wh);
	...
	private void visit(int v) {
		if (v != source) {
			...
			for (WeightedEdge edge : g.getIncidentEdges(v)) {
				 ...
				 * neighbor
				 */
				int newDValue = t.getD(v) + edge.getWeight();
				
				if (newDValue > 0 && newDValue < t.getD(edge.getTarget())) {
					ComponentTree.TreeNode wh =
						t.getUnvisitedRootOf(t, edge.getTarget());
					ComponentTree.TreeNode wi = wh.parent;
					
					int oldValue = t.getMinDviMinus(wh) >> (wi.i - 1);
					t.decreaseD(edge.getTarget(), newDValue);
					int newValue = t.getMinDviMinus(wh) >> (wi.i - 1);
					
					if (oldValue == -1 || newValue < oldValue) {
						wh.moveToBucket(wi, t.getMinDviMinus(wh) >> (wi.i - 1));
					}
				}
			}
		}
	...
	private void visit(ComponentTree.TreeNode vi) {
		...
		while (vi.numberOfUnvisitedVertices > 0 &&
			...
			while (!vi.getBucket(vi.ix).isEmpty()) {
				...
				//System.out.println(t.getD(wh.index));
				// F.3.1.2.
				visit(wh);
			}
	...
	private static class ComponentTree {
		...
		public void deleteRoot(TreeNode v) {
		...
		//	internalNodes[leaf].setParent(internalNodes[parent]);
		...
		public void decreaseD(int v, int newDValue) {
			...
			if(leafs[v].cost > newDValue&&leafs[v].NoUpdateD==false){
				leafs[v].cost = newDValue;
				TreeNode n = leafs[v].parent;
				while(n!=null&&n.NoUpdateD==false&&n.cost > newDValue){
						n.cost = newDValue;
						n = n.parent;
				}
			}
			/*
			if(leafs[v].cost > newDValue){
				leafs[v].cost = newDValue;
				TreeNode n = leafs[v].parent;
				while(n!=null&&n.cost > newDValue){
						n.cost = newDValue;
						n = n.parent;
				}
			}*/
		...
		private static class TreeNode {
			...
			/**
			 * The smallest cost of a component.
			 */
